...
router.post("/rewrite", async (req, res) => {
  ...
  const UNDETECTABLE_AI_API_KEY = '1704424452367x400168358588435900'; // Replace with your actual API key
    ...
    async function humanizeText(inputText) {
      const response = await openai.chat.completions.create({
        ...
        messages: [
          ...
          {
            role: 'user',
            content: `Humanize the text. Keep the output format i.e. the bullets and the headings as it is and dont use the list of words that are not permissible. \nTEXT: ${inputText}`,
          },
        ],
      });
