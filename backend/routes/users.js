...
router.get("/", validate, async (req, res) => {
  try {
    ...
    // Fetch rewrites to get credits
    const rewrites = await Rewrites.findOne({ userId: user._id });
    const credits = rewrites ? rewrites.rewrites : 0; // Default to 0 if no rewrites found

    // Include credits and referral code in the response
    res.send({ user: { 
      name: user.name, 
      email: user.email, 
      credits: credits, // Get credits from rewrites
...
router.post("/signup", async (req, res) => {
  ...
  try {
    ...
    // Create rewrites for the new user
    ...
    // If a referral code is provided, find the user who referred and reward them
    if (data.referralCode) {
      const referringUser = await User.findOne({ referralCode: data.referralCode });
      if (referringUser) {
        // Reward the referring user with additional rewrites
        await Rewrites.findOneAndUpdate(
          { userId: referringUser._id },
          { $inc: { rewrites: 5 } } // Reward 5 credits
        );
        ...
        const referral = new Referral({
          ...
          status: 'completed',
          rewardClaimed: false, // Set to false initially
          rewardAmount: 5
        });
    ...
    // Reward the referred user with additional rewrites
    await Rewrites.findOneAndUpdate(
      { userId: savedUser._id },
      { $inc: { rewrites: 5 } } // Reward 5 credits
    );
